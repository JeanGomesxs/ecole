...
public class TurmaRepository implements Repository<Turma,Long>{
    ...
    public Turma findById(Long id) {
        return turmas.stream().filter(turma -> turma.getId().equals(id)).findFirst().orElse(null);
    }

    @Override
    public List<Turma> findByName(String texto) {
        return turmas.stream().filter(turma -> turma.getCurso().getNome().toLowerCase().contains(texto.toLowerCase())).toList();
    }
